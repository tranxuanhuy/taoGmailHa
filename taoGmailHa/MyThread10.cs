    ...
    public class MyThread10
    {
        ...
        public static string IndividualThread(string threadPerTotalthread)
        {
            ...
            if (sw == "2")
            {
                ...
                //driver1.Quit();
                return email+"\t"+ ReadFileAtLine(1, "pvasPassword");
            }
            ...
            {
                WriteLineEmptyFile(GeneratePassword(12, 4), "pvasPassword");
                return null;
            }
            else
            {
        ...
        private static void getVercodeAPI(int time, string idphone, out string verCode, string threadPerTotalthread)
        {
            ...
            while (!responseFromServer.Contains("STATUS_OK"))
            {
                try
                {
                    ...
                    WebRequest request = WebRequest.Create("http://sms-activate.ru/stubs/handler_api.php?api_key="+ ReadFileAtLine(2, "config").Split(':')[1] +"& action=getStatus&id=" + idphone);
